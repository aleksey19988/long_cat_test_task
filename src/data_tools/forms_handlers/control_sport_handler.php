...
if (count($_POST) > 0) {
    ...
    if (strlen($sport) <= 0) {
        print_r("Не был передан вид спорта!\n");
    } else {
        $query = "SELECT * FROM sports WHERE name='{$sport}'";
        $queryResult = $conn->query($query);//Переменную $conn взяли из подключенного файла db_connect.php
        if (count($queryResult->fetchAll()) < 1) {
            print_r("В таблице не найден такой вид спорта!");
        } else {
            $query = "DELETE FROM sports WHERE name='{$sport}'";
...
$sportsList = $conn->query($query)->fetchAll();
